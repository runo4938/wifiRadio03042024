...
void setup()
{
  ...
  tft.setTextSize(1);
  tft.setFreeFont(RU8);//(&CourierCyr10pt8b);
  tft.setTextColor(TFT_CYAN);
  //tft.drawString(utf8rus(weather.name), 224, 136);

  tft.drawString(WiFi.localIP().toString(), 160, 218);
  tft.drawString(WiFi.SSID(), 2, 218);
...
void loop()
{
  ...
  if (showRadio)
  {
    ...
    { // выввод даты после меню станций
      ...
      tft.setTextColor(TFT_CYAN);
      // tft.setCursor(3, 217);
      tft.drawString(CurrentDate+utf8rus("г."), 120, 133);
      //tft.drawString(utf8rus(weather.name), 224, 136);
    ...
    {
      y1_lev = 187 - highByte(volByte); // получить число и отработать
      y2_lev = 187 - lowByte(volByte);  //
      if (y1_lev % 2 > 0) //
        y1_lev--;         // for parity
      if (y2_lev % 2 > 0)
        y2_lev--;            // на четность
...
void clock_on_core0()
{
  tft.setTextColor(0x09772, TFT_BLACK);
...
void lineondisp()
{
  ...
  tft.drawRect(70, 0, 250, 55, TFT_CYAN);
  //VuMeter
  tft.drawRect(0, 55, 70, 140, TFT_CYAN);
  //wifi level codec bitrate
  tft.drawRect(70, 123, 250, 72, TFT_CYAN);
  tft.setTextColor(TFT_CYAN);
  tft.drawString("WiFi", 90, 162);
...
void printCodecAndBitrate()
{
  ...
  tft.drawString(String(audio.getCodecname()).substring(0, 3), 194, 162);
 
  int bit = bitrate.toInt();
  if (bit < 128000)
  {
    tft.drawString(String(bit).substring(0, 2) + "k ", 250, 162);
  }
  else
  {
    tft.drawString(bitrate.substring(0, 3) + "k", 250, 162);
  }
...
void wifiLevel()
{
  uint16_t x_wifi = 140, y_wifi = ypos - 20, y_lev_wifi = 3;
  if (WiFi.RSSI() >= -60)
  {
...
//**********************************
int x_FP = 72, y_FP = ypos - 7; // position in line
//**********************************
...
void audioVolume()
{
  ...
  tft.fillRect(x_FP, y_FP, 245, 5, TFT_BLACK);
  tft.drawRect(x_FP, y_FP, 245, 6, TFT_DARKGREEN);
...
void messageOn()
{
  ...
  MessageToScroll_2 += F(" ");
  MessageToScroll_2 += weather.name;
